...
async def add_calendar_event(context, args=None):
    """
    Add an event to Google Calendar using the provided arguments.
    
    Args:
        context: The call context
        args: Dictionary containing event details (title, date, start_time, end_time, description) - 
              Not used as arguments are generated internally from the conversation context
    
    Returns:
        A confirmation message about the event creation
    """
    try:
        ...
        # This combines fragmented information across multiple turns
        user_messages = []
        ...
        try:
            ...
            # Mark function as failed to prevent immediate retry
            context.active_functions['add_calendar_event'] = {
                'status': 'failed', 
                'last_attempt': datetime.datetime.now().timestamp()
            }
            return "I couldn't process the calendar event information. Please provide all the details including title, date, start time, and end time."
        ...
        if not all([title, date, start_time, end_time]):
            ...
            return f"I need all the event details to schedule it. Please provide the following: {', '.join(missing)}."
        ...
        try:
            ...
            return f"The date format seems incorrect. Please provide the date in YYYY-MM-DD format or say the month, day and year clearly."
        
        # Log the initial request
        logger.info(f"Adding calendar event: {title} on {date} from {start_time} to {end_time}")
        
        # Set progress update
        await asyncio.sleep(1)
        context.active_functions['add_calendar_event'] = {'status': 'processing'}
        ...
        return f"I'm having trouble adding this event to your calendar. Please try again with complete details including title, date, start and end times."
